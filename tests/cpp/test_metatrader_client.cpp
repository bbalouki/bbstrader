...
TEST_F(MetaTraderClientTest, ForwardsShutdown) {
    handlers.shutdown = [&]() { generic_handler_called = true; };
    SetClientWithHandlers();
    client->shutdown();
    EXPECT_TRUE(generic_handler_called);
}
...
TEST_F(MetaTraderClientTest, ForwardsCopyRatesRangeWithDateTime) {
    ...
    client->copy_rates_range(
        symbol, 1, std::chrono::system_clock::now(), std::chrono::system_clock::now()
    );
    EXPECT_TRUE(generic_handler_called);
}
...
TEST_F(MetaTraderClientTest, ForwardsCopyTicksRangeWithDateTime) {
    ...
    client->copy_ticks_range(
        symbol, std::chrono::system_clock::now(), std::chrono::system_clock::now(), 0
    );
    EXPECT_TRUE(generic_handler_called);
}
