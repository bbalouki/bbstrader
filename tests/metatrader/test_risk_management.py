...
class TestRiskManagement(unittest.TestCase):
    ...
    def setUp(self, 
        ...
        self.symbol_info_mock = MagicMock(
            volume_step=0.01, trade_contract_size=100000, trade_tick_value=10, # tick_value_loss used by currency_risk
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.risk_manager.daily_dd, self.daily_risk) # daily_dd is the attribute name
        ...
        self.assertEqual(self.risk_manager._tf, self.time_frame) # time_frame is stored in _tf
        self.assertEqual(self.risk_manager.std, self.std_stop) # std is the attribute name
        self.assertEqual(self.risk_manager.account_leverage, self.account_leverage)
        self.assertIsNone(self.risk_manager.pchange) # pchange_sl is None by default in setUp
    ...
    def test_risk_level(self):
        ...
        with patch.object(self.risk_manager, 'get_trades_history', return_value=None) as mock_get_history:
        ...
        total_profit_calc = mock_df.commission.sum() + \
                            mock_df.fee.sum() + \
                            mock_df.swap.sum() + \
        ...
        if equity != 0:
            expected_risk_val = round((((equity - initial_balance_calc) / equity) * 100) * -1, 2)  # noqa: F841
            # (((10000 - 10083) / 10000) * 100) * -1 = 0.83

        with patch.object(self.risk_manager, 'get_trades_history', 
                          return_value=mock_df) as mock_get_history_with_df:
    ...
    def test_get_lot(self):
        ...
        self.assertEqual(self.risk_manager._volume_step(self.risk_manager.symbol_info.volume_step), 2)
        ...
        self.assertEqual(self.risk_manager._volume_step(self.risk_manager.symbol_info.volume_step), 0)
        ...
        self.assertEqual(self.risk_manager._volume_step(self.risk_manager.symbol_info.volume_step), 1)
    ...
    def test_get_pchange_stop(self):
        ...
        with patch.object(self.risk_manager, 'get_std_stop', return_value=expected_std_stop_val) as mock_get_std_stop_call:
        ...
        av_price = (self.risk_manager.symbol_info.bid + self.risk_manager.symbol_info.ask) / 2 # 1.15
        
        price_interval = av_price * (100 - pchange_value) / 100 # 1.15 * 98 / 100 = 1.127
        sl_point = float((av_price - price_interval) / point) # (1.15 - 1.127) / 0.0001 = 230.0
        sl_calc = round(sl_point) # 230
        
        deviation = self.risk_manager.get_deviation() # 1000
        min_sl_calc = self.risk_manager.symbol_info.trade_stops_level * 2 + deviation # 10 * 2 + 1000 = 1020
        expected_result_with_pchange = max(sl_calc, min_sl_calc) # max(230, 1020) = 1020
    ...
    def test_calculate_var(self):
        ...
        with patch.object(self.risk_manager, 'var_cov_var', return_value=expected_var_result) as mock_vcv:
        ...
        with patch.object(self.risk_manager, 'var_cov_var', return_value=expected_var_custom) as mock_vcv_custom:
    ...
    def test_get_stop_loss(self):
        ...
        self.assertEqual(self.risk_manager.get_stop_loss(), max(500, min_sl_expected)) # 1020
        self.risk_manager.sl = 1500
        self.assertEqual(self.risk_manager.get_stop_loss(), max(1500, min_sl_expected)) # 1500
        ...
        with patch.object(self.risk_manager, 'get_std_stop', return_value=1020) as mock_get_std_stop:
            self.assertEqual(self.risk_manager.get_stop_loss(), max(1020, min_sl_expected)) # 1020
        ...
        with patch.object(self.risk_manager, 'currency_risk', return_value=mock_currency_risk_vals) as mock_curr_risk:
            # sl_calc = round(200.0 / 2.0) = 100
            self.assertEqual(self.risk_manager.get_stop_loss(), max(100, min_sl_expected)) # 1020
        ...
        with patch.object(self.risk_manager, 'currency_risk', return_value=mock_currency_risk_vals_zero_loss) as mock_curr_risk_zero:
    ...
    def test_get_take_profit(self):
        ...
        mock_sl_value = 200
        expected_tp_calc = round(mock_sl_value * self.risk_manager.rr) # round(200 * 1.5) = 300
        with patch.object(self.risk_manager, 'get_stop_loss', return_value=mock_sl_value) as mock_get_sl:
        ...
        with patch.object(self.risk_manager, 'get_stop_loss', return_value=mock_sl_value) as mock_get_sl_2:
            self.assertEqual(self.risk_manager.get_take_profit(), expected_tp_calc_new_rr)
    ...
    def test__convert_time_frame(self):
        ...
        with patch.object(self.risk_manager, 'get_minutes', return_value=480.0) as mock_get_minutes:
    ...
    def test__check_lot(self):
        ...
        self.assertEqual(self.risk_manager._check_lot(200.0), 50.0) # Above max (returns max / 2 as per code)
    ...
    def test_get_trade_risk(self):
        ...
        with patch.object(self.risk_manager, 'risk_level', return_value=1.0) as mock_rl, \
             patch.object(self.risk_manager, 'max_trade', return_value=10) as mock_mt:
            expected_trade_risk = self.risk_manager.daily_dd / 10 
            self.assertAlmostEqual(self.risk_manager.get_trade_risk(), expected_trade_risk)
        ...
        with patch.object(self.risk_manager, 'risk_level', return_value=self.risk_manager.max_risk) as mock_rl:
            self.assertEqual(self.risk_manager.get_trade_risk(), 0) 
        with patch.object(self.risk_manager, 'risk_level', return_value=self.risk_manager.max_risk + 1) as mock_rl:
            self.assertEqual(self.risk_manager.get_trade_risk(), 0)
            
    def test_get_deviation(self):
        # self.symbol_info_mock.spread is 1000 from setUp
        self.assertEqual(self.risk_manager.get_deviation(), self.symbol_info_mock.spread)

    def test_get_break_even(self):
        # self.symbol_info_mock.spread is 1000
        original_be_attr = self.risk_manager.be # Store original be attribute from __init__
        ...
        with patch.object(self.risk_manager, 'get_pchange_stop', return_value=120) as mock_get_pchange:
